const reg1 = /[12345]/;

console.log(reg1.test("Temos o número 6"));
console.log(reg1.test("Temos o número 2"));
console.log(reg1.test("Temos o número 23"));
console.log(reg1.test("Temos o número 60"));
console.log(reg1.test("Temos o número 12345"));
console.log(("--------------"));

const reg2 = /[0-9]/;

console.log(reg2.test("Temos o número 65448484884848484"));
console.log(reg2.test("Temos o número"));
console.log(("--------------"));

const reg3 = /8080/

console.log(reg3.test("Temos a porta 8080"));
console.log(reg3.test("Temos a porta 3000"));
console.log(reg3.test("Temos a porta 3"));